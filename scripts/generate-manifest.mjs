...
const TYPE_ALIASES = new Map([
  ...
  [
    "emote",
    ["emote", "emotes", "emoticon", "emoticons", "emote-icon", "emote-icons"],
  ],
...
async function main() {
  ...
  for (const skinId of skinDirs) {
    ...
    if (meta) {
      ...
      if (Number.isInteger(meta.release_year))
        skin.release_year = meta.release_year;
    }
    ...
    for (const [canonical, aliases] of TYPE_ALIASES.entries()) {
      for (const alias of aliases) {
        ...
        // optional titles/tags overrides inside each type folder
        const titlesMap =
          (await readJson(path.join(typeDir, "titles.json"))) || {};
        ...
        await collectFiles(typeDir, (f) => {
          ...
          if (allowedByType(canonical, ext)) {
            ...
            if (canonical === "chroma" && !tags.includes("chroma"))
              tags.push("chroma");
            skin.media.push({
              type: canonical,
          ...
          await collectFiles(subDir, (f) => {
            ...
            if (allowedByType(canonical, ext)) {
              const tags = normalizeTags([
                ...subTagSlugs,
                ...inferTagsFromFilename(f),
                ...(Array.isArray(tagsMap[path.join(sub, f)])
                  ? tagsMap[path.join(sub, f)]
                  : []),
              ]);
              if (canonical === "chroma" && !tags.includes("chroma"))
                tags.push("chroma");
    ...
    if (await exists(ytTxt)) {
      ...
      for (const raw of txt
        ...
        .map((s) => s.trim())
        .filter(Boolean)) {
        const { url, inlineTitle } = parseLineWithTitle(raw);
    ...
    // Tenor GIF links via tenor.txt
    const tenorTxt = path.join(skinPath, "tenor.txt");
    if (await exists(tenorTxt)) {
      const txt = await fs.readFile(tenorTxt, "utf8");
      for (const raw of txt
        .split(/\r?\n/)
        .map((s) => s.trim())
        .filter(Boolean)) {
        ...
        const title = inlineTitle ? stripQuotes(inlineTitle).trim() : "Tenor";
        skin.media.push({
          type: "tenor",
          title,
          tenorId: id || null,
...
async function writeManifest(skins) {
  ...
  console.log(
    `Wrote ${OUT} — ${skins.length} skins, ${skins.reduce(
      (n, s) => n + s.media.length,
      0
    )} items`
  );
}

function fileBaseTitle(filename) {
  return filename
    .replace(/\.[a-z0-9]+$/i, "")
    .replace(/[-_]/g, " ")
    .trim();
...
async function collectFiles(dir, fn) {
  try {
    const e = await fs.readdir(dir, { withFileTypes: true });
    for (const x of e) {
      if (x.isFile()) fn(x.name);
    }
  } catch {}
}
...
function extractTenorId(url) {
  const s = String(url);
  const m1 = s.match(/tenor\.com\/view\/[a-z0-9-]*-([0-9]+)(?:\D|$)/i);
  if (m1) return m1[1];
  const m2 = s.match(/tenor\.com\/view\/([0-9]+)(?:\D|$)/i);
  if (m2) return m2[1];
  return null;
}
