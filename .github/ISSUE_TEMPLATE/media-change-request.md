---
name: Media Change Request
about: Request to update, add, or remove an existing media entry or suggest changes
title: Media Change Request
labels: ''
assignees: timfernix

---

### **Is your request related to an existing media?**
Please include a link or clear reference to the media entry (for example, a file name or URL).

Examples:  
- I have a higher-resolution version of XY.
- I am the original artist of one of your listed artworks and would like it removed.
- I’d like to suggest a title or tag correction for *Classic Ezreal Icon*.

Alternatively, remove this template text if your issue is something else related.

---

### **Describe the solution you'd like**
A clear and concise description of what you want to happen.  
*(e.g. Replace the current image with this higher-resolution one. / Remove the entry and credit this link instead.)*

---

### **Additional context**
Add any other context, background info, or screenshots about your request here.  
*optional*
